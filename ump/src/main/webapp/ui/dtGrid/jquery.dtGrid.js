			...
			var dtGridObject = {
				...
				load : function(){
					...
					if(!dtGridReflectionObj.option.ajaxLoad||dtGridReflectionObj.option.loadAll){
						...
						pager.parameters = options.params;
				...
				constructGrid : function(){
					...
					if(dtGridReflectionObj.option.showHeader!=false){
						...
						for(var i=0; i<columns.length; i++){
							gridContent += '	<th columnId="'+columns[i].id+'" class="'+dtGridReflectionObj.getColumnClassForHide(columns[i])+' '+columns[i].headerClass+' can-sort">';
									...
									if(value1&&value2){
										...
										if(dtGridReflectionObj.sortParameter.sortType==2){
											if(value1.time > 0){
												...
												return value2.time-value1.time;
											}else{
					...
					$('#dt_grid_print_execute_'+dtGridReflectionObj.option.id).click(function(){
						...
						if(dtGridReflectionObj.option.showHeader!=false){
							...
							$('input[id*=dt_grid_print_check_'+dtGridReflectionObj.option.id+'_]:checked').each(function(){
								...
								content += '<th class="'+dtGridReflectionObj.option.columns[columnNo].headerClass+'">'+dtGridReflectionObj.option.columns[columnNo].title+'</th>';
							});
							content += '	</tr>';
							content += '</thead>';
						}
...
(function($) {
	$.fn.DtGrid.defaultOptions = {
		grid : {
			...
			tableStyle : '',
			tableClass : 'table striped hovered',
			showHeader : true,

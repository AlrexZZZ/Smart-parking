...
(function ($) {
  ...
  $.widget("ui.customSelect", {
    ...
    friendlyVal: function () {
      return this.root.find('input:checked+span').map(function () {
    ...
    _createFromSelect: function () {
      ...
      this.element.children().each(function () {
        ...
        var element = this,
            ...
            label = '<span for="' + id + '">' + element.innerHTML + '</span>',
            input;
        var input_id=self.rootId+'_';	
        if(element.innerHTML!='全部'){
        if (self.isMultiple) {
          input = $('<input id="' + input_id+element.value + '" type="checkbox" value="' + element.value + '">');
        } else {
          input = $('<input id="' + input_id+element.value + '" name="' + self.rootId + '_radio" type="radio" value="' + element.value + '">');
        }

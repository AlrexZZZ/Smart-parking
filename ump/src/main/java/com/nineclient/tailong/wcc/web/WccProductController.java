...
import com.nineclient.tailong.wcc.dto.WccTproductsDTO;
...
@RequestMapping("/wccProduct")
@Controller
public class WccProductController {
	...
	public void loadCulture(String dtGridPager,HttpServletRequest request,Model model,HttpServletResponse response) throws UnsupportedEncodingException, ParseException {
		...
		try {
        	...
        	QueryModel<WccTproducts> qm = new QueryModel<WccTproducts>();
        	...
        	PageModel <WccTproducts> p =	WccTproducts.getRecord(qm);

        	CacluatePage.totalPage(p.getPageSize(), p.getTotalCount());
        	List<WccTproducts> lit =	p.getDataList();

        List<WccTproductsDTO> list = new ArrayList<WccTproductsDTO>();
        if(null != lit && lit.size() > 0 ){
        for(WccTproducts s:lit){
...
list.add(new WccTproductsDTO(s.getId(),s.getThumbImage(),
		s.getProductName(),s.getBackImage(),s.getpInfo(),s.getpAdvantage(),
		s.getApplayStep(), plf,s.getInsertTime(),s.getProductType()));
        }
        		
       }
        List<WccTproductsDTO> dto=new ArrayList<WccTproductsDTO>();
...
//		uiModel.addAttribute("platformUser", platformUser);
    ...
    public String createForm(HttpServletRequest request,Model uiModel,
       ...
       if(null != dataId && !"".equals(dataId)){
    	   WccTproducts w  = new WccTproducts();
    	   WccTproducts wccTproducts = w.findWccTproducts(Long.parseLong(dataId));
    	   String pltIds = "";
    	      if(null != wccTproducts.getPlatformUsers()){
    	    	  
    	        	Set <WccPlatformUser> ss  =	wccTproducts.getPlatformUsers();
    	        	Iterator<WccPlatformUser> it = ss.iterator();
    	        	while (it.hasNext()){ 
    	        		WccPlatformUser u = it.next();
    	        	     if(null != u.getId() && !"".equals(u.getId())){
    	   ...
    	   uiModel.addAttribute("wccProduct", wccTproducts);
    	   
       }
        return "tailong/create";
    }
    ...
    public String create(@Valid WccTproducts wccTproducts, BindingResult bindingResult,
...
//		PubOperator pub = CommonUtils.getCurrentPubOperator(httpServletRequest);
	...
	if(null != wccTproducts.getId() && !"".equals(wccTproducts.getId())){

		//先查询出实体
		WccTproducts wccTproductsUpdate = WccTproducts.findWccTproducts(wccTproducts.getId());
		wccTproductsUpdate.setPlatformUsers(wccTproducts.getPlatformUsers());
		wccTproductsUpdate.setThumbImage(wccTproducts.getThumbImage());
		wccTproductsUpdate.setBackImage(wccTproducts.getBackImage());
		wccTproductsUpdate.setProductName(wccTproducts.getProductName());
		wccTproductsUpdate.setpInfo(wccTproducts.getpInfo());
		wccTproductsUpdate.setpAdvantage(wccTproducts.getpAdvantage());
		wccTproductsUpdate.setApplayStep(wccTproducts.getApplayStep());
		wccTproductsUpdate.setProductType(wccTproducts.getProductType());
		 uiModel.asMap().clear();
		 //再更新数据
		 wccTproductsUpdate.merge();
		}else{
			uiModel.asMap().clear();
			wccTproducts.setInsertTime(new Date());
			wccTproducts.persist();
		}

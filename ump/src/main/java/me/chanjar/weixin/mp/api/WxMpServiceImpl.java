...
public class WxMpServiceImpl implements WxMpService {
  ...
  protected HttpClientBuilder httbuilder;
  ...
  public String getAccessToken(boolean forceRefresh) throws WxErrorException {
	  String accesstoken="";
	  try {
		  accesstoken=TokenManager.getOfficialToken(wxMpConfigStorage.getAppId(), wxMpConfigStorage.getSecret(),false);
			 	
	} catch (Exception e) {
		System.out.print("Tommy接口获取Token失败  appid="+wxMpConfigStorage.getAppId()+" and appsecret="+wxMpConfigStorage.getSecret());
		e.printStackTrace();
	}
	  return accesstoken;
...
*/
  ...
  protected synchronized <T, E> T executeInternal(RequestExecutor<T, E> executor, String uri, E data) throws WxErrorException {
    ...
    T returnT=null;
    try {
      System.out.println("uriWithAccessToken--————————————===="+uriWithAccessToken);
      returnT=executor.execute(getHttpclient(), httpProxy, uriWithAccessToken, data);
    } catch (WxErrorException e) {
      WxError error = e.getError();
      System.out.println(error.getErrorCode()+"json="+error.getJson());
      /*
       * 发生以下情况时尝试刷新access_token
       * 40001 获取access_token时AppSecret错误，或者access_token无效
       * 42001 access_token超时
       */
    if (error.getErrorCode() == 42001 || error.getErrorCode() == 40001||error.getErrorCode() == 41001) {
    	  System.out.println("___________error code =42001 or 40001");
    	  TokenManager.getOfficialToken(wxMpConfigStorage.getAppId(),wxMpConfigStorage.getSecret(),true);
    	  try {
    		  returnT= executor.execute(getHttpclient(), httpProxy, uriWithAccessToken, data);
  ...
  public void setWxMpConfigStorage(WxMpConfigStorage wxConfigProvider) {
    ...
    if (wxConfigProvider.getSSLContext() != null){
      ...
      httbuilder.setSSLSocketFactory(sslsf);
    }
  }

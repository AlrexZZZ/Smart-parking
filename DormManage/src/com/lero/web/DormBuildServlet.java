...
public class DormBuildServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if("preSave".equals(action)) {
			...
			return;
		} else if("drawMap".equals(action)){
			dormBuildList(request, response);
			return;
	...
	private void dormBuildDelete(HttpServletRequest request,
		...
		try {
			...
			if(dormBuildDao.existManOrDormWithId(con, dormBuildId)) {
				request.setAttribute("error", "站点下有所属楼层或店管，不能删除该站点");
			} else {
	...
	private void dormBuildSave(HttpServletRequest request,
		...
		if(StringUtil.isNotEmpty(lng)){
			dormBuild.setLng(Double.parseDouble(lng));
		}
	   if(StringUtil.isNotEmpty(lat)){
		   dormBuild.setLat(Double.parseDouble(lat));
		}
		Connection con = null;
		try {
			con = dbUtil.getCon();
	...
	private void dormBuildList(HttpServletRequest request,
			HttpServletResponse response)throws ServletException, IOException {
		request.setCharacterEncoding("utf-8");
		response.setCharacterEncoding("utf-8");
			...
			try {
				con = dbUtil.getCon();
				List<DormBuild> listDorm =  dormBuildDao.dormBuildAllList(con);
				JSONArray jr = JSONArray.fromObject(listDorm);
				
				pr = response.getWriter();
				pr.write(jr.toString());

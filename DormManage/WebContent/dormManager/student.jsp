...
<div class="data_list">
		...
		<form name="myForm" class="form-search" method="post" action="student?action=search" style="padding-bottom: 0px">
				<span class="data_search">
					...
					<option value="dorm" ${searchType eq "dorm"?'selected':'' }>所属楼</option>
					</select>
			...
			<table class="table table-striped table-bordered table-hover datatable">
				...
				<c:forEach  varStatus="i" var="student" items="${studentList }">
					<tr>
						...
						<td>${student.tel }</td>
						<td>
							<button class="btn btn-mini btn-success" type="button" onclick="javascript:window.location='record?action=preSave&studentNumber=${student.userName }'">添加站点出勤管理</button>
						</td>
					</tr>
				</c:forEach>
				</tbody>
